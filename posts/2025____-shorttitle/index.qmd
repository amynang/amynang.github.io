...
draft: true
draft-mode: unlinked
description: "modelling censored proportional data with brms+"
...
```{r }
#| eval: false
ycens | cens(censor, upper)
```

where `censor` is the type of censoring and `upper` is the upper bound for interval censored observations. What goes into `ycens` depends on the type of censoring, as described just above.
...
and therefore, on the left-hand side of the model formula we would have:

```{r}
#| eval: false
damagelow | cens(censor, damagehigh)
```

It turns out that this approach, and specifically those arbitrary thresholds, can get us into [trouble](https://discourse.mc-stan.org/t/posterior-predictive-check-for-zero-inflated-interval-censored-beta/39881/5). We can try to avoid the problem by making these thresholds *really* close to zero and one (`1e-7` and `1-1e-7` perhaps). But a more reliable way to proceed here, is to treat our \>25% observations as *right* censored and, perhaps less intuitively, our non-zero \<5% observations as *left* censored:
