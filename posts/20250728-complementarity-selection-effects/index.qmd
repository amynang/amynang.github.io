...
Let us now consider deviations from that null expectation; a species' yield in a mixture can be higher than the null expectation, in which case $\Delta RY_i^j > 0$ or lower, so $\Delta RY_i^j < 0$. In practice, these are never going to be exactly zero because, even in the absence of diversity effects, there would be some variation in how much a species yields in each experimental unit. But a more ecologically interesting source of deviation is if some species over or underperform in the presence of other species. And if they do, can these deviations be related to the species' intrinsic performance? Or are they purely a mixture property? There will be some variation in species' $\Delta RY_i^js$ in a given mixture; these may both positive and negative or be on one side of zero only. Species will also vary in their monoculture yield. So the next thing we are going to calculate, is the covariance between species' monoculture yield $M_i$ and their $\Delta RY_i^j$: $$\operatorname{cov}(\mathbf{M}^j, \boldsymbol{\Delta} \mathbf{RY}^j)$$ where $\mathbf{M}^j$ is the vector of monoculture yields for all species in mixture $j$ and $\boldsymbol{\Delta} \mathbf{RY}^j$ is the vector of their deviations from the expected yield. This covariance will be positive if the species with higher monoculture yield are also those with higher $\Delta RY_i^js$, and negative if these are the species that have lower $\Delta RY_i^js$. It will be close to zero if monoculture yield tells us nothing about how different species deviate from the expected yield.
...
By its mathematical definition, the covariance we just calculated is a per species quantity. We can multiply it again by $N^j$, the number of species that comprise the mixture, to get a community level quantity. This quantity is what @loreau2001 defined as the **selection effect**: $$SE^j = N^j \cdot \operatorname{cov}(\mathbf{M}^j, \boldsymbol{\Delta} \mathbf{RY}^j)$$
...
There, we have now put everything together.

Now that all the bits are explained and everything is in its right place, let's move them around a bit to understand the logic of this partitioning. Before we do that, it will help to know that the expected value of the product of two random variables is equal to the product of the expected values of each variable plus their covariance:

$$
\mathbb{E}[X \cdot Y] = \mathbb{E}[X] \cdot \mathbb{E}[Y] + \text{Cov}(X, Y)
$$

For a concrete example of this, consider several populations of a species, each with its own abundance *X* and average bodymass *Y*. If there is a negative correlation between abundance and average bodymass, then populations with high abundance are expected to have lower average bodymass. So, the expected biomass of a population (*i.e.* the biomass of the average population) is equal to the expected abundance (the abundance of the average population) multiplied by the expected average bodymass (the average bodymass of the average population) plus the (negative) covariance between population abundances and average bodymasses. Now let's look at our yield difference formula, rearranged:

$$\frac{\Delta Y^j}{N^j} =\overline{M^j} \cdot  \overline{\Delta RY^{j}} + \operatorname{cov}(\mathbf{M}^j, \boldsymbol{\Delta} \mathbf{RY}^j)$$

On the left hand side, we have the per-species difference in yield—the community-level difference in yield from the null expectation divided by the number of species. On the right, we have the product of the monoculture yield of the average species in community $j$ and the difference in relative yield of the average species, plus the covariance between monoculture yields and differences in relative yield for all species in $j$.
